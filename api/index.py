...
# Removed unsplash placeholder to avoid external API calls.
PLACEHOLDER_URL = None
SPOTIFY_CLIENT_ID = os.getenv("SPOTIFY_CLIENT_ID") or ""
SPOTIFY_SECRET_ID = os.getenv("SPOTIFY_SECRET_ID") or ""
...
def makeSVG(data: Dict[str, Any], background_color: str, border_color: str) -> str:
    barCount = 84
    contentBar = generate_bar_html(barCount)
    ...
    if item["album"]["images"] == []:
        ...
        # Use built-in placeholder image bytes for palette generation
        barPalette = generate_color_palette(None, 4, PLACEHOLDER_IMAGE)
